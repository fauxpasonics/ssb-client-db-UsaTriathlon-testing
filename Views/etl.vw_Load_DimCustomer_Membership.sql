...
CREATE VIEW [etl].[vw_Load_DimCustomer_Membership] AS 
...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
		SELECT
			...
			, 'Membership' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(pro.UserID AS NVARCHAR(50)) [SSID] -- Required
			, CAST('Member' AS NVARCHAR (50)) AS [CustomerType]
			...
			, CAST(pro.BirthDate AS DATETIME) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
			...
			/*Name*/
			, NULL AS [Prefix]
			, pro.FirstName AS [FirstName]
			, NULL AS [MiddleName]
			, pro.LastName AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
			, CASE WHEN ISNULL(pro.Address2,'')<>'' THEN pro.Address1+' '+pro.Address2 WHEN ISNULL(pro.Address2,'')='' THEN pro.Address1 END AS [AddressPrimaryStreet]
			, pro.City AS [AddressPrimaryCity] 
			, st.statename AS [AddressPrimaryState] 
			, ZipCode AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(COALESCE(pro.WorkPhone,pro.HomePhone,pro.MobilePhone),25) AS [PhonePrimary]
			, LEFT(pro.HomePhone,25) AS [PhoneHome]
			, LEFT(pro.MobilePhone,25) AS [PhoneCell]
			, LEFT(pro.WorkPhone,25) AS [PhoneBusiness]
			, LEFT(NULL,25) AS [PhoneFax]
			, LEFT(NULL,25) AS [PhoneOther]

			/*Email*/
			, CAST(COALESCE(pro.AEmail1,pro.AEmail2) AS NVARCHAR(256)) AS [EmailPrimary]
			...
			, 'Mem-' + CAST(pro.Userid AS NVARCHAR(50) ) AS [ExtAttribute6] --nvarchar(100)
			...
			, pro.CreationUserID [SSCreatedBy] --createdby
			, NULL [SSUpdatedBy] --lastmodifiedbyid
			, pro.CreationDT SSCreatedDate  
			, NULL SSUpdatedDate
			, GETDATE() CreatedDate	
			, GETDATE() UpdatedDate
			, 0 AS IsDeleted
			, NULL [AccountId]
			, 0 AS IsBusiness
			, 'MEM-' + CAST(pro.userid AS NVARCHAR(50)) AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
...
--		select top 100 *			
	...
	ON pro.StateID  = st.StateID
	LEFT JOIN dbo.BaseData_Countries ct WITH (NOLOCK)
	ON ct.CountryID = pro.CountryID
	INNER JOIN dbo.Portal_Users pu WITH (NOLOCK)
	ON pro.UserID = pu.UserID
	--LEFT JOIN dbo.[Address] addr ON addr.UserId = pro.UserID
	WHERE 
		  pro.TerminationDT > GETDATE()  AND 
		  pu.TerminationDT > GETDATE()
		AND 1=1
		AND pro.CreationDT >= DATEADD(dd,-10,GETDATE())
