...
CREATE VIEW [etl].[vw_Load_DimCustomer_Results] AS 
...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
		...
		DISTINCT
			...
			, 'Results' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(o.resultid AS NVARCHAR(50)) [SSID] -- Required
			, CAST('Result' AS NVARCHAR (50)) AS [CustomerType]
			...
			/*AddressPrimary*/
			,  o.Address AS [AddressPrimaryStreet]
			, o.City AS [AddressPrimaryCity] 
			, o.state AS [AddressPrimaryState] 
...
--		select top 100 *			
	FROM	 dbo.Results_Result o
	INNER JOIN dbo.Results_Race r ON o.RaceId = r.RaceId
	WHERE o.MemberId IS NULL
		AND 1=1
		AND r.[Date]  >= DATEADD(dd,-30,GETDATE())
