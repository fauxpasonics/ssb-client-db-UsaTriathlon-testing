...
CREATE VIEW [etl].[vw_Load_DimCustomer_Marketo] AS 
...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
		SELECT
			...
			, 'Marketo' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			...
			, CAST(CASE WHEN memberid <> '' THEN 'Member' ELSE 'Non_Member' END AS NVARCHAR (50)) AS [CustomerType]
			...
			, TRY_CAST(mkt.dateOfBirth AS DATETIME) AS [Birthday] -- field is sensitive see writup in sharepoint for an example of how complicated it can get
			, mkt.Gender AS [Gender] 
			, 0 [MergedRecordFlag]
			, NULL [MergedIntoSSID]
			...
			/*Name*/
			, NULL AS [Prefix]
			, mkt.FirstName AS [FirstName]
			, NULL AS [MiddleName]
			, mkt.LastName AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
			, mkt.address AS [AddressPrimaryStreet]
			, mkt.City AS [AddressPrimaryCity] 
			, mkt.inferredStateRegion AS [AddressPrimaryState] 
			, mkt.postalCode AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(mkt.mainphone,25) AS [PhonePrimary]
			, LEFT(mkt.phone,25) AS [PhoneHome]
			, LEFT(mkt.mobilePhone,25) AS [PhoneCell]
			, LEFT(NULL,25) AS [PhoneBusiness]
			, LEFT(NULL,25) AS [PhoneFax]
			, LEFT(NULL,25) AS [PhoneOther]

			/*Email*/
			, CAST(mkt.Email AS NVARCHAR(256)) AS [EmailPrimary]
			...
			, 'PLACE_HOLDER' [ExtAttribute6] --nvarchar(100)
			...
			, mkt.membershipExpirationDate AS [ExtAttribute21] --datetime
			...
			, mkt.createdAt AS SSCreatedDate  
			, mkt.UpdatedAt SSUpdatedDate
			, GETDATE() CreatedDate
			, GETDATE() UpdatedDate
			, 0 AS IsDeleted
			, NULL [AccountId]
			, 0 AS IsBusiness
			, 'Mem'+'-'+mkt.memberId AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/

--		select top 100 *			
	FROM ods.Marketo_Leads mkt
	WHERE 1=1
--	AND mkt.createdAt >= DATEADD(dd,-15,GETDATE())
--	OR 
--	mkt.UpdatedAt >= DATEADD(dd,-15,GETDATE())
