...
CREATE VIEW [etl].[vw_Load_DimCustomer_Licenses] AS 
...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
		SELECT
			...
			, 'Licenses' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			...
			, CAST('Licensee' AS NVARCHAR (50)) AS [CustomerType]
...
--		select top 100 *			
	...
	WHERE o.UpgradedMembershipId IS NULL
		AND 1=1
	AND a.ResultDate >= DATEADD(dd,-15,GETDATE())
