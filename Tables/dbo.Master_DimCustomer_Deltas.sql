...
GO
CREATE NONCLUSTERED INDEX [IX_Master_DimCustomer_Deltas_DimCustomerId] ON [dbo].[Master_DimCustomer_Deltas] ([DimCustomerId]) INCLUDE ([ElementID], [Field], [ProcessedDate]) WITH (DATA_COMPRESSION = PAGE)
GO
CREATE NONCLUSTERED INDEX [IX_Master_DimCustomer_Deltas_InsertDate] ON [dbo].[Master_DimCustomer_Deltas] ([InsertDate] DESC) WITH (DATA_COMPRESSION = PAGE)
GO
CREATE CLUSTERED INDEX [IX_Master_DimCustomer_Deltas_ProcessedDate_DimCustomerId] ON [dbo].[Master_DimCustomer_Deltas] ([ProcessedDate], [DimCustomerId]) WITH (DATA_COMPRESSION = PAGE)
GO
